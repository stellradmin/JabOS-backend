-- CORRECTED V3 FINAL: Fix CTE scope issue in FOR loop
-- The compatibility_calculated CTE is not accessible outside RETURN QUERY
...
BEGIN
    ...
    -- Step 1: Get potential matches and calculate compatibility
    FOR match_record IN
        SELECT
            p.id,
            p.display_name,
    ...
    LOOP
        -- Check cache first
        SELECT cs.compatibility_score INTO compatibility_value
        FROM compatibility_scores cs
        WHERE (cs.user_id = viewer_id AND cs.potential_match_id = match_record.id) OR
              (cs.user_id = match_record.id AND cs.potential_match_id = viewer_id)
        ORDER BY cs.calculated_at DESC
        LIMIT 1;

        -- If not cached, calculate now
        IF compatibility_value IS NULL THEN
            BEGIN
                compatibility_result := calculate_compatibility_scores(viewer_id, match_record.id);
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    RAISE WARNING 'Compatibility calculation failed for % and %: %', viewer_id, match_record.id, SQLERRM;
                    compatibility_value := 75.0;
            END;
        END IF;

        -- Set values for RETURNS TABLE output
        id := match_record.id;
        display_name := match_record.display_name;
        avatar_url := match_record.avatar_url;
        gender := match_record.gender;
        age := match_record.age;
        zodiac_sign := match_record.zodiac_sign;
        interests := match_record.interests;
        education_level := match_record.education_level;
        bio := match_record.bio;
        compatibility_score := COALESCE(compatibility_value, 75.0);
        distance_km := match_record.distance_km;
