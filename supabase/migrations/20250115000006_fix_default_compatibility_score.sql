-- Fix default compatibility score from 75.0 to NULL
-- This forces fresh calculation instead of showing misleading default scores
...
BEGIN
    ...
    potential_matches AS (
        ...
        WHERE
            ...
            -- Match's gender preference (bidirectional)
            AND (
    ...
    SELECT
        ...
        pm.education_level,
        pm.bio,
        -- CRITICAL FIX: Return NULL instead of 75.0 default
        -- Edge function will ALWAYS calculate fresh scores
        -- Cached scores are only returned if recently calculated (within expiry)
        (SELECT cs.compatibility_score
         ...
         AND cs.expires_at > NOW() -- Only use non-expired cached scores
         ORDER BY cs.calculated_at DESC
         LIMIT 1) as compatibility_score,
        pm.distance_km
    FROM potential_matches pm
    ORDER BY
        -- NULL scores will sort last, ensuring calculated scores appear first
        COALESCE(
            (SELECT cs.compatibility_score
             FROM compatibility_scores cs
             WHERE (cs.user_id = viewer_id AND cs.potential_match_id = pm.id) OR
             ...
             ORDER BY cs.calculated_at DESC
             LIMIT 1),
            0 -- Use 0 for sorting only (NULL scores appear last)
        ) DESC,
        pm.id -- Secondary sort by ID for consistency
    LIMIT limit_count
    OFFSET offset_count;
END;
$$;
...
'FIXED VERSION - Returns NULL for compatibility_score when not cached:
- NULL scores force Edge Function to calculate fresh scores
- Cached scores only used if within expiry window
- Edge function ALWAYS calculates fresh scores for accuracy
- Activity, zodiac, age, gender preference filtering
- Swipe exclusion via cached table';
