...
-- Drop existing policies on match_requests (if table exists)
DO $$ BEGIN
...
-- Create consolidated, efficient RLS policies for match_requests (if table exists)
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'match_requests') THEN
        -- 1. SELECT: Users can view match requests they're involved in
        EXECUTE 'CREATE POLICY "match_requests_select_policy" ON public.match_requests
            FOR SELECT
            USING (
                auth.role() = ''service_role''::text
        ...
        EXECUTE 'CREATE POLICY "match_requests_insert_policy" ON public.match_requests
            FOR INSERT
            WITH CHECK (
                auth.role() = ''service_role''::text
        ...
        EXECUTE 'CREATE POLICY "match_requests_update_policy" ON public.match_requests
            FOR UPDATE
            USING (
                auth.role() = ''service_role''::text
                OR auth.uid() = requester_id
        ...
        EXECUTE 'CREATE POLICY "match_requests_delete_policy" ON public.match_requests
            FOR DELETE
            USING (
                auth.role() = ''service_role''::text
